...
paste("PCR MSE on test set =", round(pcr.mse))
@
...
@
<<echo=FALSE>>=
paste("Lasso MSE on test set =", lasso.mse)
@

Looking at the plots produced by both methods we can denote some degree of agreeability between the two, as both require more than 30 components/co-variates in order to minimize the MSE (to value approximately equal to 4).
...
Alternatively, given that our output is ordered we may think about applying ordered logistic regression.
...
We can compute the confidence intervals 
<<echo=FALSE, out.width='11cm', out.height='11cm', fig.cap='Ratio between coefficients obtained at the 2.5 percent and 97.5 percent quantiles. Negative values indicate that the confidence interval contains zero and therefore is not an effective predictor.', fig.align="center">>=
