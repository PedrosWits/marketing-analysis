...
We start by splitting the data into training and test sets and performing principal component analysis. This enables us to understand how many components are required to explain the majority of variance in the data.
<<size="small">>=
set.seed(1337)
...
<<echo=FALSE, size="small", out.width='8cm', out.height='8cm', fig.show='hold', fig.pos='H', fig.cap='Histograms for true and predicted values of Income using PCR'>>=
hist(y[test], main="True", xlab = "Income", col="purple")
hist(pcr.pred, main="Predicted", xlab = "Income", col="gray")
@
...
Still, it is of interest to analyse the residuals to see if these provide a good represention of the random and unpredictable characteristics of the error term.
...
<<echo=FALSE, size="small", out.width='8cm', out.height='8cm', fig.show='hold', fig.pos='H', fig.cap='Histograms for true and predicted values of Income using Lasso'>>=
hist(y[test], main="True", xlab = "Income", col="purple")
...
olr.model = polr(as.ordered(as.ordered(y[train])) ~ x[train,],Hess=TRUE)
olr.cis = confint(olr.model)
olr.predict = as.numeric(predict(olr.model, x[test,]))
olr.mse=mean((y[test]-olr.predict)^2)
@

<<echo=FALSE>>=
paste("MSE test for ordered logistic regression model =", olr.mse)
@

<<echo=FALSE>>=
olr.cis.ratio = olr.cis[,1]/olr.cis[,2]
names(olr.cis.ratio) = substring(rownames(olr.cis),11)
...
<<echo=FALSE, size="small", out.width='8cm', out.height='8cm', fig.show='hold', fig.pos='H', fig.cap='Histograms for true and predicted values of Income using ordered logistic regression'>>=
hist(y[test], main="True", xlab = "Income", col="purple")
hist(olr.predict, main="Predicted", xlab = "Income", col="gray")
@
